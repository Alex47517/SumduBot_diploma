...
class Bot {
    ...
    public static function request($method, $params, $associative = false) {
        ...
        $url = 'https://api.telegram.org/bot'.self::$token.'/'.$method.'?'.$params_string;
        $response = json_decode(file_get_contents($url, false, $context), $associative);
        if (!$response->ok && !$associative) {
            if (stristr($response->description, 'parse entities')) {
                unset($params['parse_mode']);
                return self::request($method, $params, $associative);
            }
