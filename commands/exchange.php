...
function sendOffers($type, $page) {
    ...
    if (!$offers[0]['id']) custom_error('Біржа', 'Наразі оголошень нема. Скористуйтеся командою <code>!конвертувати</code> або створіть своє оголошення');
    if (count($offers)%5 < $page && update::$btn_id) { $chat->answerCallbackQuery('💢 Ви переглянули усі сторінки', true); die(); }
    $echo = 0;
