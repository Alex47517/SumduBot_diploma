...
Виправте помилку або напишіть /start для виходу');
        ...
        $vars = ['id', 'nick', 'rank', 'balance', 'group', 'tg', 'date', 'achievements'];
        foreach ($vars as $var) {
...
У тебе: <b>'.$user->user['diamonds'].'</b>💎', null, update::$btn_id);
                ...
                if (!$user->user['custom_info']) {
                    ...
                    $custominfo->text = $user->user['tmp'];
                    R::store($custominfo);
                    if (!R::findOne('userachievements', 'user_id = ? AND achievement_id = ?', [$user->user['id'], 14])['id']) {
                        $user->getAchievement(14);
