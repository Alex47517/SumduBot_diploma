...
if ($cmd[1] == 'гроші' or $cmd[1] == 'money' or $cmd[1] == 'деньги') {
    $default_limit = 5;
    if ($cmd[2]) $limit = round($cmd[2]); else $limit = $default_limit;
    if (!$limit) $limit = $default_limit;
    $top_users = array_values(R::find('users', 'ORDER BY `balance_usd` DESC LIMIT ?', [$limit]));
    $text = '💵 <b>Топ по грошам:</b>
';
    foreach ($top_users as $key => $top_user) {
        $text .= getEmojiNum(($key+1)).' <a href="tg://user?id='.$top_user['tg_id'].'">'.$top_user['nick'].'</a> -> <b>'.$top_user['balance_usd'].' 💵</b>
';
    }
    $chat->sendMessage($text);
}
